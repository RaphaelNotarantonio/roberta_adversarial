...
def main(): 
    ...
    def perturb_iterative_fool_many(xvar, embvar, indlistvar, yvar, predict, nb_iter, eps, epscand, eps_iter, loss_fn,rayon,
      ...
      while ii<nb_iter and not(fool):
          ...
          loss = loss_fn(outputs, yvar) - 0.1*modd(inputs_embeds=embvar+delta,labels=torch.tensor([1]).to(device))[0] 
          if minimize:
              loss = -loss 
    ...
    for uid in range(3):
      ...
      for eps_iter in [2.]:  #attention à totalnbphrase
        ...
        #JUST ADD SMTHING TO TRANSFORM INTO A SENTENCE
        for ba in range(batch_sze):
          nwi=''
          for w in new_wordbatch[ba]:
           nwi=nwi+w
          new_wordbatch[ba]=[nwi]
